...
if [ "${CHARLIE}" = "yes" ]; then
    nsplug meta_vehicle_fld.moos targ_$VNAME3.moos -f       \
        VNAME=$VNAME3                                     \
	VPORT=$VPORT3                                     \
	WARP=$WARP                                        \
	...
	COOL_STEPS=$COOL_STEPS				  \
   	CONCURRENT=$CONCURRENT  			  \
	ADAPTIVE=$ADAPTIVE				  \
	START_POS=$RETURN_PT3

    nsplug meta_vehicle.bhv targ_$VNAME3.bhv -f             \
        VNAME=$VNAME3                                     \
	CRUISESPEED=$CRUISESPEED                          \
        START_POS=$RETURN_PT3                             \
...
if [ "${DAVIS}" = "yes" ]; then
    nsplug meta_vehicle_fld.moos targ_$VNAME4.moos -f       \
        VNAME=$VNAME4                                     \
	VPORT=$VPORT4                                     \
	WARP=$WARP                                        \
	...
	COOL_STEPS=$COOL_STEPS				  \
   	CONCURRENT=$CONCURRENT  			  \
	ADAPTIVE=$ADAPTIVE				  \
	START_POS=$RETURN_PT4

    nsplug meta_vehicle.bhv targ_$VNAME4.bhv -f             \
        VNAME=$VNAME4                                     \
	CRUISESPEED=$CRUISESPEED                          \
        START_POS=$RETURN_PT4                             \
